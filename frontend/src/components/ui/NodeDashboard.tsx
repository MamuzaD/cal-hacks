...
export function NodeDashboard({ node, allNodes, allEdges }: NodeDashboardProps) {
  const navigate = useNavigate()

  // Helper to get ID from edge source/target (handles both string and D3 object format)
  const getEdgeId = (value: any): string => {
  ...
  const incomingEdges = connectedEdges.filter((edge) => getEdgeId(edge.target) === node.id)
  const outgoingEdges = connectedEdges.filter((edge) => getEdgeId(edge.source) === node.id)

  const handleSearch = () => {
    // Navigate to the visual page for this node
    const nodeSlug = node.label.toLowerCase().replace(/[^a-z0-9]+/g, '-')
    navigate({ to: `/visual/${nodeSlug}` })
  }

  const getNodeLabel = (nodeId: string) => {
    const foundNode = allNodes.find((n) => n.id === nodeId)
    return foundNode?.label || nodeId
  }
  ...
  const getEdgeTypeColor = (type: string) => {
    // Single types
    if (type === 'stock-holding') return 'bg-green-500/20 text-green-400 border-green-500/30'
    if (type === 'campaign-contribution') return 'bg-red-500/20 text-red-400 border-red-500/30'
    if (type === 'lobbying') return 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30'
    if (type === 'investment') return 'bg-blue-500/20 text-blue-400 border-blue-500/30'
    ...
    if (type.includes(' + ')) {
      ...
      if (type.includes('stock-holding') && type.includes('campaign-contribution'))
  ...
  return (
    <div className="h-full flex flex-col p-4 bg-background border-l border-primary/20 overflow-hidden">
      ...
      <div className="flex-shrink-0 mb-3">
        <h2 className="text-xl font-bold mb-2 break-words line-clamp-2">{node.label}</h2>
        <div className="flex flex-col gap-1.5 mb-3">
          ...
          {node.category && (
            <span className="text-sm text-muted-foreground line-clamp-1">{node.category}</span>
          )}
        </div>
              ...
              {outgoingEdges.map((edge, idx) => (
                ...
                >
                  <div className="flex items-start gap-2 mb-1.5">
                    ...
                    <span className="font-medium text-sm break-words flex-1 line-clamp-2">
